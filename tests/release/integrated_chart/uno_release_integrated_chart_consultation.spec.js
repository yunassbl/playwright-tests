...
test('test', async ({ page }) => {
  ...
  // 상담 진입
  await expect(page.getByText('상담 (0)')).toBeVisible();
  ...
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '해피콜' })).toBeVisible();
  ...
  // 펜차트 > 펜차트 샘플함
  await expect(page.getByLabel('펜차트 샘플함').getByText('펜차트 샘플함')).toBeVisible();
  await expect(page.getByLabel('고객상담차트.jpg').getByRole('checkbox')).toBeVisible();
  ...
  await expect(page.locator('[role="combobox"]').nth(15)).toBeVisible();
  ...
  await expect(page.locator('[role="combobox"]').nth(16)).toBeVisible();
  await page.locator('[role="combobox"]').nth(16).click();
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '눈수술' })).toBeVisible();
  ...
  await page.getByRole('combobox', { name: '-' }).click();
  await page.waitForTimeout(1000);
  await expect(page.getByRole('option', { name: '환불' })).toBeVisible();
  await page.getByRole('option', { name: '환불' }).click();
  await page.waitForTimeout(1000);
  // 상담 내용 수정
  await expect(page.locator('label').filter({ hasText: '상담내용' })).toBeVisible();
  await expect(page.getByText('자주 쓰는 상용구')).toBeVisible();
  await expect(page.locator('pre').getByText('상담 내용 입력 자동화')).toBeVisible();
  ...
  // 펜차트 > 펜차트 샘플함
  await expect(page.getByLabel('펜차트 샘플함').getByText('펜차트 샘플함')).toBeVisible();
  await expect(page.getByLabel('강쥐.jpg').getByRole('checkbox')).toBeVisible();
  await page.getByLabel('강쥐.jpg').getByRole('checkbox').check();
  await page.waitForTimeout(1000);
  await expect(page.getByLabel('펜차트 샘플함').getByText('펜차트 샘플함')).toBeVisible();
  await expect(page.getByLabel('고양이(1).jpeg').getByRole('checkbox')).toBeVisible();
  await page.getByLabel('고양이(1).jpeg').getByRole('checkbox').check();
  await page.waitForTimeout(1000);
  await expect(page.getByLabel('펜차트 샘플함').getByText('펜차트 샘플함')).toBeVisible();
  await expect(page.getByLabel('ray.jpg').getByRole('checkbox')).toBeVisible();
  ...
  await expect(page.locator('div').filter({ hasText: /^ray\.jpg$/ }).getByRole('button').nth(1)).toBeVisible();
