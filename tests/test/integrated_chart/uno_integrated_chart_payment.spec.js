...
test('test', async ({ page }) => {
  ...
  await expect(page.getByRole('cell', { name: '0', exact: true })).toBeVisible(); // 할인
  ...
  await expect(page.getByRole('cell', { name: '수납메모 > 수납대기' })).toBeVisible(); // 수납 메모 
  await expect(page.getByRole('cell', { name: '김정연' }).nth(1)).toBeVisible(); // 상담사
  await expect(page.getByRole('cell', { name: '김정연' }).nth(2)).toBeVisible(); // 담당자
  // 수납대기 카테고리 선택
  ...
  // 저장 안내 팝업
  await expect(page.getByText('수납일: 2025-02-')).toBeVisible();
  await expect(page.getByText('담당자: 간편접수')).toBeVisible();
  ...
  await expect(page.getByRole('cell', { name: '간편접수' })).toBeVisible(); // 담당자
  // 결제 영역
  await expect(page.getByRole('cell', { name: '수납', exact: true })).toBeVisible(); // 구분
  ...
  await page.getByRole('button', { name: '저장' }).click();
  // 저장 안내 팝업
  await expect(page.getByText('수납일: 2025-02-13담당자: 간편접수위 내용으로 결산/통계 내용이 업데이트 됩니다')).toBeVisible();
  await page.getByRole('button', { name: '확인' }).click();
  await expect(page.getByText('수납을 수정했습니다. 연결된 접수정보가 업데이트 됩니다')).toBeVisible();
  await expect(page.getByRole('cell', { name: '등록된 내용이 없습니다' })).toBeVisible();
  // 수납차트 > 완납 선택
