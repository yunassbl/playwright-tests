...
test('test', async ({ page }) => {
  await page.goto('https://test.unocare.co.kr/login'); // 로그인 화면
  ...
  await expect(page1.getByRole('cell', { name: '고객명_자동화' })).toBeVisible();
  await expect(page1.getByRole('cell', { name: 'dev', exact: true })).toBeVisible();
  await expect(page1.getByRole('cell', { name: '- 1234568' }).getByRole('textbox').first()).toBeVisible();
  ...
  await page1.getByRole('row', { name: '등록 1 -1 루루카캡슐75밀리그램(프레가발린)_(' }).getByRole('cell').nth(1).click();
  ...
  // 진료 등록 확인
  await expect(page.getByRole('cell', { name: '진료 내용 입력 자동화' })).toBeVisible(); // 진료 메모
  ...
  await page.getByRole('button', { name: 'icon-image 펜차트 샘플함' }).click();
  await expect(page.getByLabel('펜차트 샘플함').getByText('펜차트 샘플함')).toBeVisible();
  await page.getByLabel('kha(7).png').getByRole('checkbox').check();
  await page.getByLabel('kha copy(3).png').getByRole('checkbox').check();
  await page.getByLabel('kha copy 2(3).png').getByRole('checkbox').check();
  await page.getByRole('button', { name: '불러오기' }).click();
  ...
  await expect(page.getByRole('cell', { name: 'nt2' }).nth(2)).toBeVisible(); // 의사 수정
  await expect(page.getByRole('cell', { name: '성형' }).nth(2)).toBeVisible(); // 시/수술 카테고리 수정
  await expect(page.getByRole('cell', { name: '피부 시술' })).toBeVisible(); // 시/수술명 수정
  await expect(page.getByRole('cell', { name: '눈매교정' })).toBeVisible(); // 시/수술 카테고리 추가 수정
  await expect(page.getByRole('cell', { name: '인모드' })).toBeVisible(); // 시/수술명 추가 수정
